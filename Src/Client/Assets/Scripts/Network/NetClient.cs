...
namespace Network
{
    class NetClient : MonoSingleton<NetClient>
    {
        ...
        public bool Connected
        {
            get
            {
                //Socket是类，这里实际是clientSocket != null
                return (clientSocket != default(Socket)) && clientSocket.Connected; //上一次收发是否完成
            }
        }
        ...
        {
            running = true;
            MessageDistributer.Instance.ThrowException = true;  //用于打印错误消息
        }
        ...
        void DoConnect()
        {
            ...
            try
            {
                ...
                {
                    Blocking = true  //设置阻塞（等待服务器响应）
                };

                Debug.Log(string.Format("Connect [{0}] to server {1}", this.retryTimes, this.address) + "\n");
                IAsyncResult result = this.clientSocket.BeginConnect(this.address, null, null); //连接结果
                bool success = result.AsyncWaitHandle.WaitOne(NetConnectTimeout);   //等待1s
                if (success)
                {
        ...
        #region 供服务层调用，发送消息
        /// <summary>
        /// 发送消息(send a Protobuf message，还没正式发，只是消息入队)
        /// </summary>
        ...
        public void Update()
        {
            ...
            {
                if (this.ProcessRecv()) //是否接收到数据
                {
                    if (this.Connected) //防止接收完毕后断线
                    {
                        this.ProcessSend(); //向服务器发送请求消息（send）
                        this.ProcessMessage();   //向客户端派发响应消息（委托）
                    }
                }
            }
        }
        ...
        bool ProcessRecv()
        {
            try
            {
                ...
                bool error = this.clientSocket.Poll(0, SelectMode.SelectError); //Poll: 检查 Socket 的状态
                if (error)
                {
                ...
                bool ret = this.clientSocket.Poll(0, SelectMode.SelectRead); //可读
                if (ret)
                {
        ...
        bool ProcessSend()
        {
            try
            {
                ...
                bool ret = this.clientSocket.Poll(0, SelectMode.SelectWrite);   //检查Socket是否为可写状态（true:连接成功/可以发送数据）
                if (ret)
                {
                    ...
                    else
                    {
                        ...
                        {
                            NetMessage message = this.sendQueue.Peek(); //获取队列开始处的对象而不删除它
                            byte[] package = PackageHandler.PackMessage(message);       //封包
